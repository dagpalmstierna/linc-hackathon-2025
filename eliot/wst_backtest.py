...
def compute_scattering_features(signal, scattering):
    ...
    # Compute scattering coefficients and flatten the result to 1D.
    features = scattering(x)
    return features.squeeze().flatten()


def prepare_ml_data(pivot_df, window_size, scattering):
    """
    ...
    Returns feature matrix X, target vector y, list of timestamps, and corresponding tickers.
    """
    X_list, y_list, time_list, ticker_list = [], [], [], []
    
    for ticker in pivot_df.columns:
        prices = pivot_df[ticker].values  # price series for a given stock
        ...
        for t in range(window_size, len(prices) - 1):
            ...
            # Skip windows with no variation (std=0) to avoid division by zero.
            if std_val == 0:
                continue
            # Normalize the window to mitigate scale differences.
            window = (window - np.mean(window)) / std_val
            features = compute_scattering_features(window, scattering)
            # If scattering features contain NaN, skip this sample.
...
def ml_trading_strategy(df, window_size=24, capital=1000000, train_ratio=0.7):
    """
    ...
    # Initialize the scattering transform with chosen parameters.
    J = 3  # scale parameter
    Q = 1  # quality factor
    scattering = Scattering1D(J=J, shape=window_size, Q=Q)
    
    # Build the ML dataset.
    X, y, times, tickers = prepare_ml_data(pivot_df, window_size, scattering)
    ...
    times_train, times_test = np.array(times[:split_idx]), np.array(times[split_idx:])
...
# Execute the ML trading strategy backtest.
if __name__ == "__main__":
    ...
    model, portfolio_df, test_results = ml_trading_strategy(df, window_size=24, capital=1000000, train_ratio=0.7)
