...
def create_index_fund(df, capital=1000000, index_only=False):
    ...
    if index_only:
        ...
        for idx, row in initial_prices.iterrows():
            ...
            shares = int(allocation // ask_price)
...
def plot_portfolio(portfolio_history, portfolio):
    ...
    and plot the value of each individual holding in a separate window.
    """
...
if __name__ == "__main__":
    df = load_csv_to_df('./data/Historical_Data.csv')
    ...
    save_portfolio_history(portfolio_history, './data/index_portfolio.csv')
