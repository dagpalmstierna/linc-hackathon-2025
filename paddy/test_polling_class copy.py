...
class DataPoller:
    def __init__(self, polling_intervals: Dict[str, float]):
        ...
        for func_name in polling_intervals.keys():
            # Create a dedicated cache variable for each function
            setattr(self, f"{func_name}_cache", None)
            # Create a dedicated lock for each function
            setattr(self, f"{func_name}_lock", threading.Lock())
    ...
    def _poll_data(self, func_name: str, interval: float):
        ...
        while self.running:
            ...
            try:
                result = getattr(lh, func_name)()  # Call the blocking API function
                print(f'Polled {func_name}')
                with getattr(self, f"{func_name}_lock"):  # Lock only this function's cache
                    setattr(self, f"{func_name}_cache", result)
    ...
    def get_cached_data(self, func_name: str) -> Any:
        cache_var = getattr(self, f"{func_name}_cache")
        return cache_var
        # lock = getattr(self, f"{func_name}_lock")
        # with lock:  # Lock only this function's cache
        #     return cache_var
...
if __name__ == "__main__":
    ...
    try:
        while True:
            func_name = random.choice(list(polling_intervals.keys()))
